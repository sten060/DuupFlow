...
export default function TikTokGuideClient() {
  ...
  return (
    <main className="relative p-6">
      ...
      <style>{`
        .guide-prose h3{font-size:1.15rem;font-weight:700;color:var(--app-text);margin:1.8rem 0 .55rem}
        .guide-prose p{color:var(--app-text-muted);line-height:1.75;margin:.6rem 0}
        .guide-prose ul,.guide-prose ol{color:var(--app-text-muted);line-height:1.75;margin:.6rem 0 .6rem 1.25rem}
        .guide-prose ul{list-style:disc}
        .guide-prose ol{list-style:decimal}
        .guide-prose li{margin:.3rem 0}
        .guide-prose strong{color:var(--app-text);font-weight:600}
      `}</style>
      ...
      <div className="relative mx-auto flex max-w-5xl gap-10">
        ...
        <article className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-4">
            <Link
              ...
              className="text-sm text-[var(--app-text-muted)] transition hover:text-[var(--app-text-muted)]"
            >
          ...
          <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-[var(--app-text)] md:text-4xl">
            {t("dashboard.tiktokGuide.title")}
          </h1>
          <p className="mt-2 max-w-2xl text-[var(--app-text-muted)]">{t("dashboard.tiktokGuide.subtitle")}</p>
          ...
          <div className="mt-12 space-y-16">
            {TIKTOK_GUIDE.map((c) => (
              <section key={c.id} id={c.id} className="scroll-mt-24">
                ...
                <h2 className="mt-1.5 text-2xl font-extrabold tracking-tight text-[var(--app-text)] md:text-3xl">
                  {c.title[locale]}
                </h2>
                <p className="mt-1.5 text-[var(--app-text-faint)]">{c.lead[locale]}</p>
        ...
        <aside className="hidden w-56 shrink-0 lg:block">
          <div className="sticky top-8">
            <div className="mb-3 text-xs font-semibold uppercase tracking-wider text-[var(--app-text-faint)]">
                ...
                <li key={c.id}>
                  <button
                    ...
                    className={`flex w-full items-baseline gap-2 rounded-md border-l-2 px-3 py-1.5 text-left text-sm transition ${
                      active === c.id
                        ? "border-sky-400 bg-sky-400/10 text-[var(--app-text)]"
                        : "border-transparent text-[var(--app-text-muted)] hover:text-[var(--app-text-muted)]"
                    }`}
                  >
