  ...
  return (
    <section className="mt-2">
      ...
      <div className="mb-5">
        ...
        <p className="text-sm text-[var(--app-text-faint)] mt-1">
      ...
      >
        ...
        <span className="text-base text-[var(--app-text-muted)]">tokens</span>
        <span className="ml-auto text-xs text-[var(--app-text-faint)]">{t("dashboard.tokens.available")}</span>
      </div>

      {/* Topup packs */}
      <div className="mb-6">
        <h3 className="text-sm font-semibold text-[var(--app-text)] mb-3">{t("dashboard.tokens.buyTitle")}</h3>
          ...
          {TOPUP_PACKS.map((pack) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-1 text-xl font-bold text-[var(--app-text)]">
                  {tokens} tokens
                </p>
                {bonusTokens && (
                ...
                <p className="mt-2 text-xs text-[var(--app-text-muted)]">{formatEur(pack.priceCents)}</p>
              </button>
            );
          })}
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="flex gap-2 max-w-md">
            <div className="relative flex-1">
              <input
                ...
                className="w-full rounded-lg border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-2 pr-10 text-sm text-[var(--app-text)] outline-none focus:border-emerald-400/40"
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-[var(--app-text-faint)]">€</span>
            </div>
            <button
              type="button"
          ...
          <p className="mt-1.5 text-[11px] text-[var(--app-text-faint)]">
      ...
      <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)]">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-[var(--app-surface)] transition rounded-xl"
        >
          <span className="text-sm font-semibold text-[var(--app-text)]">
            {t("dashboard.tokens.historyTitle")}
            {state?.ledger?.length ? (
              <span className="ml-2 text-xs text-[var(--app-text-faint)] font-normal">
                ({state.ledger.length})
              </span>
            ) : null}
          </span>
          <svg
            viewBox="0 0 12 12"
            className={`h-3 w-3 text-[var(--app-text-faint)] transition-transform ${historyOpen ? "rotate-180" : ""}`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
          >
        ...
        {historyOpen && (
          <div className="border-t border-[var(--app-border)] px-4 py-3">
            {loading ? (
              <p className="text-xs text-[var(--app-text-faint)]">{t("dashboard.tokens.loading")}</p>
            ) : !state || state.ledger.length === 0 ? (
              <p className="text-xs text-[var(--app-text-faint)]">{t("dashboard.tokens.noTx")}</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="text-left text-[var(--app-text-faint)] border-b border-[var(--app-border)]">
                    ...
                    {state.ledger.map((row) => {
                      ...
                      return (
                        <tr key={row.id} className="border-b border-[var(--app-border)] last:border-0">
                          <td className="py-2 pr-4 text-[var(--app-text-muted)] whitespace-nowrap">
                          ...
                          <td className="py-2 pr-4 text-[var(--app-text-muted)]">{row.reason}</td>
