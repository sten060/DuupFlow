...
export default function AbonnementClient({
  ...
  const [freeCheckoutLoading, setFreeCheckoutLoading] = useState<"solo" | "pro" | null>(null);

  // Free → Solo/Pro flow: opens Stripe Checkout. The webhook (api/stripe/webhook
  // `checkout.session.completed`) flips `has_paid + plan` and credits welcome
  // tokens automatically — no further client action needed after redirect.
  async function startFreeCheckout(targetPlan: "solo" | "pro") {
    setFreeCheckoutLoading(targetPlan);
    setMsg(null);
    try {
      ...
      if (res.ok && data.url) {
        ...
        setMsg({ type: "err", text: data.error ?? t("dashboard.subscription.networkError") });
  ...
  return (
    ...
    {/* Free → Solo/Pro upgrade modal — Free users pick a plan and get
        redirected to Stripe Checkout. After payment, the webhook flips
        their plan + has_paid + credits welcome tokens automatically. */}
    {showFreeUpgradeModal && (
      <div
        className="fixed inset-0 z-50 flex items-center justify-center p-4"
        style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}
        onClick={() => freeCheckoutLoading === null && setShowFreeUpgradeModal(false)}
      >
        <div
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              type="button"
              onClick={() => freeCheckoutLoading === null && setShowFreeUpgradeModal(false)}
              disabled={freeCheckoutLoading !== null}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <button
                type="button"
                onClick={() => startFreeCheckout("solo")}
                disabled={freeCheckoutLoading !== null}
            ...
            >
              ...
              <button
                type="button"
                onClick={() => startFreeCheckout("pro")}
                disabled={freeCheckoutLoading !== null}
                className="w-full rounded-xl py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50"
                style={{ background: "linear-gradient(135deg,#6366F1,#38BDF8)" }}
              >
                {freeCheckoutLoading === "pro"
                  ? "Redirection vers Stripe…"
                  : "Choisir Pro"}
              </button>
            </div>
